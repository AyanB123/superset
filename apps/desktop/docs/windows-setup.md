...
- **a. Agent bootstrap hooks are NOT installed on Windows.** The activity-tracking hooks for Claude / Codex / Cursor / Gemini / Copilot are bash scripts (`#!/bin/bash` with `/tmp/`, `grep`, `tr`, `date -u`). On Windows the setup-target allowlist (`WINDOWS_SUPPORTED_AGENT_SETUP_TARGET_IDS`) is empty, so these hooks are skipped during agent bootstrap. Agent status/notifications that depend on these hooks will not fire.
...
- **e. Distributable CLI packaging (`bun run build` → `bundle:cli`, and the standalone `superset` CLI exe) is blocked by a Bun-on-Windows tooling bug.** Bun's JS-API `Bun.build({ target: "bun", compile, plugins })` reports success but writes no output file on Windows (the commands plugin is the trigger; the CLI-form `bun build --compile` is unaffected). `cli-framework build` now fails loudly when the artifact is missing. This does **not** affect running the app in dev, the native terminal stack, or typecheck/lint — only producing the packaged `.exe`. It needs an upstream Bun fix or a deeper workaround. Separately: if Bun is installed via npm (`bun.cmd` shim) **and** the official installer (`~/.bun/bin/bun.exe`), the shim shadows the real binary and breaks `bun build --target=<…>` compiles with "bun is not installed in %PATH%". Prefer the official installer; `apps/desktop/scripts/build-bundled-cli.ts` already prefers `~/.bun/bin`.
