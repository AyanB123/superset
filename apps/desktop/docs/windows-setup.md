...
- **a. Agent bootstrap hooks: Claude works on Windows; others are pending.** The notify hook (`notify.sh`) is bash and runs under Git Bash. Claude's hooks are installed on Windows — `~/.claude/settings.json` points at a per-agent `claude-notify.cmd` launcher (in `$SUPERSET_HOME_DIR/hooks`) that runs `notify.sh` via Git Bash. Claude launched inside a Superset terminal inherits the `SUPERSET_*` env (terminal id, hook URL) from the terminal, so activity notifications fire. Requires Git for Windows (Git Bash) installed. Codex / Cursor / Gemini / Copilot hooks are not yet ported (each needs its own Windows hook-command strategy) — they're skipped via `WINDOWS_SUPPORTED_AGENT_SETUP_TARGET_IDS`.
